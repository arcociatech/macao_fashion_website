        ...
        if ($request->type == "Physical") {
            ...
            if (empty($request->color_check)) {
                ...
                if (!empty($request->color_image)) {
                    for ($i = 0; $i < count($request->color_image); $i++) {
                        $thumbnail = Storage::put('/assets/images/products/colors', $request->color_image[$i]);
                        $color_image_path[$i] = $thumbnail;
                    }
                    $input['color_image'] = implode(',', $color_image_path);
                }
                // if (empty($request->color)) {
                //     $input['color_image'] = null;
                // }
            }
