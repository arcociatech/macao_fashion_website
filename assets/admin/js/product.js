(function($) {
    ...
    $("#color-btn").on('click', function() {
        ...
        $("#color-section").append(
            ...
            '<div class="input-group mb-3 colorpicker-component cp"> <div class = "custom-file" ><input name="color_image[]" type="file" class = "custom-file-input" id= "inputGroupFile01" aria-describedby="inputGroupFileAddon01"> <label class="custom-file-label"for="inputGroupFile01"> Choose file <label> </div> </div><hr></div > ');
        $('.cp').colorpicker();
    });
    ...
    $(document).on('click', '.color-remove', function() {
        ...
        if (isEmpty($('#color-section'))) {
            ...
            $("#color-section").append('<div class="color-area">' +
                '<span class="remove color-remove"><i class="fas fa-times"></i></span>' +
                '<input type="text" placeholder="Enter color name" name="color[]" class="form-control" />' +
                '</div>' +
